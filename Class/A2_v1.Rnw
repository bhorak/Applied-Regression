...
Here we see a very messy pairs plot. As this data set has a great meany variables
...
\textbf{B:} Fit Multiple regrresion model relation CO2 product to total solvent and hydrogen
consumption.
<<results=tex>>=
...
The F test of significanse yields an F Statisitc of probablity of \Sxpr{summary(fit)$fstatistic[1]}
with a corresponding p value of \Sxpr{p}. The R-squared value comes out to \Sxpr{summary(fit)$r.squared}.

\textbf{D:} Now do the test in (c) using t tests and determine the contribution of the two regres- sors.
<<results=tex>>=
print(xtable(summary(fit)))
@
...
\textbf{E:} Construct the 95% CI on the regression parameters in (b).
...
f <- ms/(ssr/dfr)
f[3]<-0
P <- pf(f, degf, dfr, lower.tail = FALSE)
P[3]<-0
...
For comparison here is the anova table that are produces

<<results=tex>>=
print(xtable(anova(fit)))
@

As you can see both the tables are identical.
\end{document}